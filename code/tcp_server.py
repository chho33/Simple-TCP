...
def send_ack(packet, client_address, client_port):
    serverSocket = socket(AF_INET, SOCK_DGRAM)
    serverSocket.sendto(packet, (client_address, client_port))
    serverSocket.close()
...
def receiver():
    global window, window_size, mask_for_leftmost, mask_for_window, cur_window_start, FIN, file_content, file_path, server_port
    serverSocket = socket(AF_INET, SOCK_DGRAM)
    ...
    while not FIN:
        ...
        if content["error"]:
            ...
            #except ValueError:
            #    continue
            #send_ack(packet, client_address[0], client_port)
            continue
        ...
        bits = bits_for_update_window(cur_window_start, seq_no, len(data), window_size)
        window |= bits
        count = 0
...
if __name__ == '__main__':
    ...
    mask_for_leftmost = "0" + "1"*(window_size-1)
    mask_for_leftmost = int(mask_for_leftmost, 2)
    mask_for_window = "1" + "0"*(window_size)
    mask_for_window = int(mask_for_window, 2)
